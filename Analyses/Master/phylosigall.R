#phylogenetic signal in SPCIS all taxa
...
d.samp2<-dplyr::select(d,SpCode,NativeStatus,NA_L1NAME,Plot)
d.samp2 <-d.samp2%>% dplyr::group_by(NA_L1NAME,SpCode,NativeStatus) %>% dplyr::count()
d.samp2<-distinct(d.samp2)

d.samp2$nonnative<-ifelse(d.samp2$NativeStatus=="I",1,0)
d.samp2$nonnative<-d.samp2$nonnative*d.samp2$n
...
USA.ses.mntd<-ses.mntd(L1, phy.dist, null.model = "taxa.labels", abundance.weighted = FALSE, runs = 999)
USA.ses.mntdabn<-ses.mntd(L1abn, phy.dist, null.model = "taxa.labels", abundance.weighted = TRUE, runs = 999)
USA.ses.mdpabn<-ses.mpd(L1abn, phy.dist, null.model = "taxa.labels", abundance.weighted = TRUE, runs = 999)
...
mntd.plot.smallabn<-ggplot(mntd.small,aes(reorder(NA_L1NAME,Estimate),Estimate))+
  geom_errorbar(aes(ymin=lower,ymax=upper,color=cat),width=0.0,size=.5)+
  #geom_errorbar(aes(ymin=lower2,ymax=upper2,color=cat),width=0,size=.5,linetype="dotdash")+
  geom_point(aes(shape=cat,color=cat),size=4)+
  scale_x_discrete(name="")+ggthemes::theme_few(base_size = 9)+
  scale_shape_manual(name="",values=c(16,1))+scale_color_manual(name="",values=c("black","black"))+
  theme(legend.position = "top")+ylab("Mean nearest taxon distance")+coord_cartesian(ylim=c(0,100))



USA.ses.mntd<-USA.ses.mdpabn
uno<-data.frame(NA_L1NAME=c(rownames(USA.ses.mntd)),Estimate=c(USA.ses.mntd$mpd.rand.mean))

uno$lower<-c(uno$Estimate-USA.ses.mntd$mpd.rand.sd)
uno$upper<-c(uno$Estimate+USA.ses.mntd$mpd.rand.sd)
uno$lower2<-c(uno$Estimate-2*USA.ses.mntd$mpd.rand.sd)
uno$upper2<-c(uno$Estimate+2*USA.ses.mntd$mpd.rand.sd)

uno$metric<-c("MPD")
uno$cat<-"randomly generated"

uno2<-data.frame(NA_L1NAME=c(rownames(USA.ses.mntd)),Estimate=USA.ses.mntd$mpd.obs)
uno2$lower<-uno2$Estimate
uno2$upper<-uno2$Estimate
uno2$lower2<-uno2$Estimate
uno2$upper2<-uno2$Estimate
...
pdf
mpd.small<-filter(uno,NA_L1NAME %in% c('EASTERN TEMPERATE FORESTS','GREAT PLAINS','NORTH AMERICAN DESERTS','NORTHERN FORESTS','NORTHWESTERN FORESTED MOUNTAINS'))
mpd.plot.smallabn<-ggplot(mpd.small,aes(reorder(NA_L1NAME,Estimate),Estimate))+
  geom_errorbar(aes(ymin=lower,ymax=upper,color=cat),width=0.0,size=.5)+
  #geom_errorbar(aes(ymin=lower2,ymax=upper2,color=cat),width=0,size=.5,linetype="dotdash")+
  geom_point(aes(shape=cat,color=cat),size=4)+
  scale_x_discrete(name="")+ggthemes::theme_few(base_size = 9)+
  scale_shape_manual(name="",values=c(16,1))+scale_color_manual(name="",values=c("black","black"))+
  theme(legend.position = "top")+ylab("Mean phylogenetic distance")+coord_cartesian(ylim=c(200,400))+theme(axis.text.x = element_blank(),axis.ticks.x = element_blank())

pdf("Analyses/Master/Plots/invasiveclusters.pdf")
ggpubr::ggarrange(mpd.plot.smallabn,mntd.plot.smallabn,common.legend = TRUE,ncol=1)
dev.off()
